...
审计之外还有一层同样重要的东西：**输入冻结、排他 lease、三类 failure marker、可回滚的原子提升**
——它们保证「产物文件存在 == 本次通过」。这些机制的实现细节（以及它们**不能**证明什么）
见 [docs/lifecycle.md](docs/lifecycle.md)。
...
这两套 GT 该怎么标注、怎么按父视频隔离、最低样本量是多少、报哪些分层指标、
什么状态才算数，写在 [docs/evaluation.md](docs/evaluation.md)。
...
- [生命周期与审计实现细节](docs/lifecycle.md)：为什么输入要冻结成拷贝而不是硬链接、
  排他 lease 与三类 failure marker、可回滚的原子提升、独立双遍渲染的三条帧链、
  strict-audit 的参数预注册、稳定问题摘要、shadow 私有生命周期、调度接力的「一次一项」；
- [数据获取与评测方法论](docs/evaluation.md)：两套独立 GT、父视频隔离、最低样本量、
  四引擎最小运行矩阵、分层指标与状态机、素材目录与入库门禁、采集批次与停止条件、
  公开数据集的权利结论；
- [工程约束与优化记录](docs/optimization.md)：版本演进与实测数字、四个最小复现与修复、
  增强视图三轮记录、量化与端侧约束、模型替换与许可边界、以及明确不做的清单；
- [机器链规模与队列快照](docs/scale.md)：目录规模、场景矩阵状态、各长片 strict 跑出的
  轨迹与队列量级、教师侧运行规模、三教师汇总状态与恢复阻塞清单。

> 读这些文档时请记住两条口径：**规模不等于通过**（`unresolved_count` 是待复核项计数，
> 不是漏牌数），以及**每个数字都带「测什么 / 样本 / 状态」**——
> 没有样本与状态的数字不要拿来当结论。
